...
def get_artist(artist_id: int, db: Session = Depends(get_db)):
    ...
    artist_dict = ArtistOut.model_validate(a).model_dump()
...
def list_shows(city: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    return json_ok([ShowOut.model_validate(s).model_dump() for s in shows])
...
def get_show(show_id: int, db: Session = Depends(get_db)):
    ...
    return json_ok(ShowOut.model_validate(s).model_dump())
